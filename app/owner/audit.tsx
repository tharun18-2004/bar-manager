    ...
    <div className="flex h-screen bg-black text-white">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="bg-zinc-900 border-b border-zinc-800 flex gap-6 px-6">
          <Link
            href="/owner"
            className="px-6 py-4 font-semibold uppercase text-sm transition border-b-2 border-transparent text-zinc-400 hover:text-white"
          >
            Owner Dashboard
          </Link>
          <span className="px-6 py-4 font-semibold uppercase text-sm transition border-b-2 border-blue-500 text-blue-400">
            Audit Log
          </span>
        </div>
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6 mb-6">
            ...
            <div className="flex items-center justify-between mt-4">
              <p className="text-xs text-zinc-500">Showing up to {auditLimit} records per page.</p>
              <div className="flex gap-2">
                <button
                  onClick={handleAuditPrevious}
                  disabled={auditLoading || auditPageStack.length === 0}
                  className="px-3 py-1.5 rounded bg-zinc-800 border border-zinc-700 text-sm disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  Previous
                </button>
                <button
                  ...
                  className="px-3 py-1.5 rounded bg-blue-600 text-sm disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  Next
                </button>
              </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6">
            <div className="space-y-4">
              {auditLoading && <p className="text-zinc-500 text-center py-8">Loading audit logs...</p>}
              {auditError && <p className="text-red-400 text-center py-8">{auditError}</p>}
              {!auditLoading && !auditError && auditLogs.length === 0 && (
                <p className="text-zinc-500 text-center py-8">No audit records found</p>
              )}
              {!auditLoading &&
                !auditError &&
                auditLogs.map((entry, index) => (
                  <div
                    ...
                    className="border-l-4 border-blue-500 pl-4 py-2 bg-blue-900 bg-opacity-20 rounded px-4"
                  >
                    <p className="font-bold text-blue-400">{entry.action}</p>
                    ...
                    <p className="text-zinc-500 text-xs">Time: {new Date(entry.created_at).toLocaleString()}</p>
                  </div>
                ))}
            </div>
          </div>
